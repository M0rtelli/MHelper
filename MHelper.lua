...
style()		
			if not povs.v or not cheats.v or not help.v or not main_window_state.v or not lvla.v or not gps1.v or not gps2.v or not gps3.v or not gps4.v or not gps5.v or not gps6.v or not gps7.v or not gps8.v or not sec_window_state.v or not main_menu.v or not slave.v then
imgui.Process = false
end
...
if panel.v then
	...
	if isKeyJustPressed(VK_F5) or wh.v then 
	wh.v = not wh.v
	...
	if isKeyJustPressed(VK_INSERT) or gm.v then -- GM
	gm.v = not gm.v
	...
	if isKeyJustPressed(VK_RSHIFT) or air.v then -- airbrk
	air.v = not air.v
	...
	if not isGMeh then
		if isKeyJustPressed(VK_B) then -- fastwalk
		...
		if isKeyJustPressed(VK_O) or nofall.v then -- nofall
		nofall.v = not nofall.v
			switchFunc('NoFall')
		end
	else
...
if panel.v then
	...
	if isKeyJustPressed(222) then -- unfreeze
	...
	if isGMeh then
		if isKeyJustPressed(VK_1) then -- fix
			fixCar(veh)
		end
		
		if isKeyDown(VK_LMENU) then -- speedhack
		...
		if isKeyJustPressed(VK_BACK) then -- turn FUCK THIS
			local vecx, vecy, vecz = getCarSpeedVector(veh)
		...
		if isKeyJustPressed(VK_N) then -- fast exit
...
setCharCoordinates(PLAYER_PED, x, y, z + 10)
	...
	if text:match('Connected to SanTrope') then
	lua_thread.create(function()
	checkupd("https://gist.githubusercontent.com/M0rtelli/de2309930d8201e561d018359dac76f3/raw", '{00FFFF}[MHelper]: ', "https://raw.githubusercontent.com/M0rtelli/MHelper/main/MHelper.lua")
	end)
	end
