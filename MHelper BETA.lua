...
local rdata = {}
local distances = 0
forms = {'/warn','/ban','/jail','/fv','/get','/offget','/ipaccounts','/skick','/sethp','/skin','/unwarn','/banip','/unjail','/offban','/offwarn','/spcar','/sban','/offjail','/offmute','/spcarid','/oad','/spawn'}
local conf = {
    DialogID = {
    	DID = 579
    },
...
while not isSampAvailable() do wait(50) end
	...
	applySettings(admins)
	for i=1, #forms do
avto[i] = imgui.ImBool(false)
end
for i=1, #forms do
vruchnuyu[i] = imgui.ImBool(true)
end
...
if not imgui.Process then
   ...
   sampTextdrawDelete(i)
	...
	if isKeyJustPressed(48) and not sampIsChatInputActive() and not sampIsDialogActive() and cfg.settings.aforms then
   if got == 'Нет форм для принятия.' then
      printStringNow('~r~~h~~h~Not successfully!', 3000)
   else
   sampSendChat(got, -1)
      printStringNow('~r~Successfully!', 3000)
...
if imgui.CustomButton(u8'Назад', imgui.ImVec4(0.0,0.0,0.0,1.0), imgui.ImVec4(1.0,0.0,0.0,1.0), imgui.ImVec4(1.0,0.0,0.0,1.0), imgui.ImVec2(130, 25)) then
 ...
 imgui.Process = true	
	local sg, se = getScreenResolution()
		...
		if cfg.settings.aforms == true then
		...
		cfg.settings.aforms = not cfg.settings.aforms
...
local pokaz1 = false
local autoprin = imgui.ImBool(false)
local nakazaniya = imgui.ImBuffer(512)
local formi = {}
local ac = imgui.ImBool(false)
...
imgui.Text(u8'Введите формы ниже, чтобы выдать наказания с КД 0.6 секунды. (!!ебейший флуд форм!!)')
imgui.InputTextMultiline("##пухом мне земля", nakazaniya, imgui.ImVec2(300, 375))
imgui.SameLine()
...
imgui.Text(forms[i])
imgui.NextColumn()
if conf.formsauto[i] then
avto[i].v = true
vruchnuyu[i].v = false
end
if conf.formsvr[i] then
vruchnuyu[i].v = true
avto[i].v = false
end
imgui.BeginChild('##child номер ' .. i, imgui.ImVec2(140,35))
imgui.SetCursorPosX(75)
imgui.SetCursorPosY(4)
if imgui.Checkbox(u8'##' .. i, avto[i]) then
vruchnuyu[i].v = not avto[i].v
if avto[i].v then
conf.formsauto[i] = true
conf.formsvr[i] = false
else
conf.formsvr[i] = true
conf.formsauto[i] = false
end
inicfg.save(cfg, 'MHelper.ini')
end
imgui.EndChild()
imgui.NextColumn()
imgui.BeginChild('##pizdatiy child nomer ' .. i, imgui.ImVec2(140,35))
imgui.SetCursorPosX(75)
imgui.SetCursorPosY(4)
if imgui.Checkbox(u8'##v' .. i, vruchnuyu[i]) then
avto[i].v = not vruchnuyu[i].v
if vruchnuyu[i].v then
conf.formsvr[i] = true
conf.formsauto[i] = false
else
conf.formsvr[i] = false
...
function sel6()
imgui.BeginChild(u8'цвета', imgui.ImVec2(606, 0), true)
imgui.Text(u8'бля, я настолько заебался, что не сделал это.\nтак что в след. обнове будет надеюсь.\n\np.s. я даже не продумал как это и что это блять такое :))')
imgui.EndChild()
end
function sel7()
imgui.BeginChild(u8'цвета', imgui.ImVec2(606, 0), true)
...
local formss = {u8'/warn','/kick','/ban','/mute','/jail','/fv','/get','/offget','/ipaccounts','/skick','/sethp','/skin','/unwarn','/banip','/unjail','/offban','/offwarn','/spcar','/sban','/offjail','/offmute','/spcarid','/oad','/spawn'}
	local adm, idadm, pro = text:match("%[A%] (%w+_%w+)%[(.*)%]: (.*)")
	
if pro then
	local checked = false
	for i=1, 24 do
	local konec = pro:match(formss[i])
	local com, args = pro:match('/(%a*)%s(.*)')
	if konec then 
	--sampAddChatMessage(com, -1)
	for i=1, #forms do
	if conf.formsauto[i] and forms[i]:match(com) then
	sampSendChat('/' .. com .. ' ' .. args)
	checked = true
	break
	else
	checked = false
	...
	printStyledString('NEW ADMIN FORM by~r~ \n' .. ' ' .. adm, 3000, 4)
	--printStyledString('~r~' .. adm, 3000, 4)
...
if text:match('Вы были телепортированы по метке на карте') then
local x, y, z = getCharCoordinates(PLAYER_PED)
